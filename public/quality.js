...
const QCFG = {
  ...
  student_activities:{title:'الأنشطة الطلابية',cols:['title','organizer','activity_type','date','students_count','rating'],heads:['العنوان','الجهة المنظمة','النوع','التاريخ','الطلبة','التقييم'],fields:[{l:'العنوان*',id:'title',t:'text'},{l:'الجهة المنظمة',id:'organizer',t:'select',opts:DEANSHIP_DEPTS},{l:'نوع النشاط/المبادرة/الجلسة/الحملة',id:'activity_type',t:'text'},{l:'التاريخ*',id:'date',t:'date'},{l:'عدد الطلبة',id:'students_count',t:'number'},{l:'أسماء العاملين',id:'staff_names',t:'textarea'},{l:'أسماء القيادات',id:'leaders_names',t:'textarea'},{l:'جهة خارجية',id:'external_party',t:'yesno'},{l:'اسم الجهة الخارجية',id:'ext_name',t:'text'},{l:'أسماء المشاركين من الخارج',id:'ext_people',t:'textarea'},{l:'رقم تقييم النشاط',id:'rating',t:'number'}]},
  student_activities_external:{title:'الأنشطة الطلابية الخارجية',cols:['title','organizer','activity_type','date','students_count','rating'],heads:['العنوان','الجهة المنظمة','النوع','التاريخ','الطلبة','التقييم'],fields:[{l:'العنوان*',id:'title',t:'text'},{l:'الجهة المنظمة',id:'organizer',t:'select',opts:DEANSHIP_DEPTS},{l:'نوع النشاط/المبادرة/الجلسة/الحملة',id:'activity_type',t:'text'},{l:'التاريخ*',id:'date',t:'date'},{l:'عدد الطلبة',id:'students_count',t:'number'},{l:'أسماء العاملين',id:'staff_names',t:'textarea'},{l:'أسماء القيادات',id:'leaders_names',t:'textarea'},{l:'جهة خارجية',id:'external_party',t:'yesno'},{l:'اسم الجهة الخارجية',id:'ext_name',t:'text'},{l:'أسماء المشاركين من الخارج',id:'ext_people',t:'textarea'},{l:'رقم تقييم النشاط',id:'rating',t:'number'}]},
  ...
  staff_committees:{title:'مشاركة الموظفين في اللجان المجتمعية',cols:['staff_name','workplace','committee_name','committee_type','ref_num','date'],heads:['الموظف','مكان العمل','اللجنة','النوع','رقم الكتاب','التاريخ'],fields:[{l:'اسم الموظف*',id:'staff_name',t:'text'},{l:'جهة العمل',id:'workplace',t:'text'},{l:'الصفة الوظيفية',id:'job_title',t:'text'},{l:'اسم اللجنة*',id:'committee_name',t:'text'},{l:'نوع اللجنة',id:'committee_type',t:'select',opts:['وطنية','دولية']},{l:'رقم الكتاب',id:'ref_num',t:'text'},{l:'تاريخ الاشتراك',id:'date',t:'date'}]},
...
async function loadQData(table) {
  ...
  const canEdit=canEditGlobal() || (['coordinator','manager'].includes(ME?.role) && ['student_activities','student_activities_external'].includes(table));
  ...
  tbody.innerHTML=(rows||[]).map((r,i)=>`<tr style="${r.source&&!r.completed?'background:#FFFBF0':''}">
    ...
    ${canEdit?`<td><div class="rb">
      ...
      ${['student_activities','student_activities_external','community_svc'].includes(table)?`<button class="btn btn-sm" style="background:#8B6914;color:#fff;border-color:#8B6914" onclick="openAttachModal('${table}','${r.id}','${r.name||r.title||''}')">${r.attached_participant_id?'📄 كشف مرفق':'📎 إرفاق كشف'}</button>`:''}
      ...
      ${canEditGlobal()?`<button class="btn btn-r" onclick="delRec('${table}','${r.id}',()=>loadQData('${table}'))">🗑</button>`:''}
    </div></td>`:''}
...
function showQForm(table, record=null) {
  ...
  // إزالة أي كتلة تصنيفات سابقة
  const prevCat=document.getElementById('qf-cats-'+table); if(prevCat) prevCat.remove();
...
async function loadFData(table) {
  ...
  document.getElementById('ftbl-'+table).innerHTML=(rows||[]).map((r,i)=>`<tr style="${r.source&&!r.completed?'background:#FFFBF0':''}">
    ...
    ${table==='hall_bookings' && r.request_id ? (r.confirmed?`<td><span class="st st-a" style="font-size:10px">✅ مؤكَّد</span></td>`:`<td><span class="st st-p" style="font-size:10px">⏳ محجوز مبدئياً</span></td>`)
    ...
    ${canEdit?`<td><div class="rb">
      ${r.source&&!r.completed&&!(table==='hall_bookings'&&r.request_id)?`<button class="btn btn-sm btn-g" onclick="markFComplete('${table}','${r.id}')">✅ مكتمل</button>`:''}
...
async function filterPart() {
  ...
  document.getElementById('tbl-part').innerHTML=(rows||[]).map((r,i)=>{
    ...
    <td>${r.organizer||'-'}</td><td>${total||0}<div style="font-size:10px;color:var(--muted)">حاضر: ${attended}</div>${r.email_sent?`<div style="font-size:10px;color:#27500A">✅ تم إرسال بريد (${r.email_sent_count||0})</div>`:''}</td>
    <td><div class="rb">
      ...
      ${canEditPart?`<button class="btn btn-sm" style="color:#8A2A22;border-color:#8A2A22" onclick="openSendEmailModal('${r.id}')">📧 إرسال بريد</button>`:''}
...
function openSendEmailModal(id) {
  ...
  modal.querySelector('.modal').innerHTML = `
    ...
    <div class="fg"><label>المُرسَل إليهم</label>
      <select id="se-target">
        <option value="registered">كل المسجَّلين (ممن أدخل بريده الإلكتروني)</option>
        <option value="attended">الحاضرين فقط (ممن أدخل بريده الإلكتروني)</option>
      </select>
    </div>
    <div class="fg"><label>عنوان الرسالة</label><input type="text" id="se-subject" placeholder="مثال: تذكير بموعد الرحلة"></div>
    <div class="fg"><label>نص الرسالة</label><textarea id="se-message" style="min-height:120px" placeholder="اكتب نص الرسالة هنا..."></textarea></div>
    <div id="se-msg" class="msg"></div>
    <div style="display:flex;gap:8px;margin-top:10px">
      <button class="btn" style="flex:1;background:var(--g);color:#fff" onclick="sendPartEmail('${id}')"><i class="ti ti-send"></i> إرسال</button>
      <button class="btn" onclick="document.getElementById('mod-sendemail').classList.remove('open')">إلغاء</button>
    </div>`;
  modal.classList.add('open');
  if (!window.__seEscBound) {
    window.__seEscBound = true;
    document.addEventListener('keydown', e => { if (e.key === 'Escape') document.getElementById('mod-sendemail')?.classList.remove('open'); });
  }
}

async function sendPartEmail(id) {
  const target = document.getElementById('se-target').value;
  const subject = document.getElementById('se-subject').value.trim();
  const message = document.getElementById('se-message').value.trim();
  ...
  if (!subject || !message) { show('يرجى إدخال عنوان ونص الرسالة', true); return; }
  show('جارٍ الإرسال...');
...
function buildQRowBodyHTML(table, record) {
  ...
  ${rows}
  ${(Array.isArray(record.categories)&&record.categories.length)?`<div style="margin-top:10px;border:1px solid #C6E8D3;border-radius:5px;padding:8px;background:#F0FAF4"><div style="font-weight:700;color:#1B6B3A;font-size:9pt;margin-bottom:4px">تصنيفات الجودة المعتمدة:</div>${record.categories.map((c,i)=>`<div style="font-size:8.5pt;padding:1px 0">${i+1}. ${c}</div>`).join('')}</div>`:''}
