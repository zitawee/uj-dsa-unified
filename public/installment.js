...
// القائمة تصل من الشؤون المالية بأرقام جامعية وكليات فقط، وتُلصَق هنا دفعة واحدة.
// الطالب يستعلم عن نفسه من صفحة عامة منفصلة /installment.html بلا تسجيل دخول.
// ══════════════════════════════════════════════════════════════
...
async function inClearAll() {
  ...
  const r = await api('/api/installment_plan/clear-all', 'POST');
  if (r.error) { alert('تعذّر الحذف: ' + r.error); return; }
  await loadInstallmentPlan();
  alert(r.message || 'تم حذف القائمة بالكامل');
}
