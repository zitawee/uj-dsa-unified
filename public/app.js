...
function buildSidebar() {
  ...
  <div class="folder-body sec-acts" id="f-acts" style="display:none">
    ...
    ${(ME?.role==='admin'||ME?.role==='editor') ? `<div class="ni" onclick="go('activity_instructions',this)"><i class="ti ti-speakerphone"></i>تعليمات النشاط</div>` : ''}
  </div>
...
function buildPanels() {
  ...
  const IDS = ['dash','incomplete','talent_excellence','talent_committee','room_booking','training_courses','activity_instructions','students','achievements','activity_requests','activity_requests_external','announcements','hall_bookings','participants','committees','meeting_invites','meeting_minutes','governance','student_activities','student_activities_external','student_honors','staff_committees','staff_training','staff_innovation','staff_honors','uni_committees','community_svc','reports','committee_report','sa_report','cat_report','eval_report','search','users','archive'];
...
function go(name, el) {
  ...
  const loaders = {
    dash:loadDash, incomplete:loadIncomplete, talent_excellence:loadTalent, talent_committee:loadTalentCommittee, room_booking:loadRoomBooking, training_courses:loadTrainingCourses, activity_instructions:loadInstructions, students:loadStudents, achievements:loadAchievements,
