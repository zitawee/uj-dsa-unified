...
function scOpenPrintFields(mode) {
  ...
  document.getElementById('sc-modal-body').innerHTML = `
    ...
    ${mode!=='final' ? `<label style="display:flex;align-items:center;gap:6px;font-weight:400;font-size:12.5px;margin-bottom:12px"><input type="checkbox" id="sc-print-include-finalscore"> تضمين عمود "العلامة النهائية"</label>` : ''}
...
function scPrintGradingSheet() {
  ...
  const includeFinalScore = document.getElementById('sc-print-include-finalscore')?.checked === true;
  ...
  const html = `
    ${SC_PRINT_FONT}${SP_TABLE_ALIGN_STYLE}
    <style>.ptbl th{background:#fff!important;color:#000!important;border:1px solid #333!important}.ptitle{background:#eee!important;color:#000!important}</style>
    <div class="ph2">
      <img src="/logo.png" class="plogo" alt="شعار الجامعة الأردنية">
      <div class="puni"><div class="ar">الجامعة الأردنية</div><div class="en">The University of Jordan</div><div class="dep">عمادة شؤون الطلبة — Dean of Student Affairs</div></div>
      <div class="pmeta">${spDate(new Date())}</div>
    ...
    <table class="ptbl" style="table-layout:fixed;width:100%"><thead><tr>
      ...
      ${rows.map((r,i)=>`<tr><td style="text-align:center">${i+1}</td><td style="text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${spEsc(r.full_name)}</td>${extraCols.map(c=>`<td style="text-align:center">${spEsc(spFieldValuePrint(r,c.key))}</td>`).join('')}${extraColTitles.map(()=>`<td style="height:36px"></td>`).join('')}${includeFinalScore ? `<td style="height:36px"></td>` : ''}</tr>`).join('')}
    </tbody></table>
    ${spSignatureBlockHTML()}`;
  openPrint(html);
  scCloseModal();
...
function scPrintFinalReport() {
  ...
  const html = `
    ...
    <table class="ptbl" style="table-layout:fixed;width:100%"><thead><tr>
      ...
      ${rows.map((r,i) => {
        const nom = r.nomination_score!=null ? r.nomination_score : (spNominationScore(r.nomination_type) || 0);
        return `<tr><td style="text-align:center">${i+1}</td><td style="text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${spEsc(r.full_name)}</td>${extraCols.map(c=>`<td style="text-align:center">${spEsc(spFieldValuePrint(r,c.key))}</td>`).join('')}${extraColTitles.map(()=>`<td style="height:36px"></td>`).join('')}<td style="text-align:center">${r.committee_score!=null?spPct(r.committee_score):'—'}</td><td style="text-align:center">${r.hs_score!=null?spPct(r.hs_score):'—'}</td><td style="text-align:center">${spPct(nom)}</td><td style="font-weight:700;text-align:center">${r.final_score!=null?spPct(r.final_score):'—'}</td></tr>`;
      }).join('')}
    </tbody></table>
    ${spSignatureBlockHTML()}`;
  openPrint(html);
